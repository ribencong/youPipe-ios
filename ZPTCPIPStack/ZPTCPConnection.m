    ...
    if (self) {
        ...
        if (_block->tcpInfo.flags & TCP_ACK) {
            ...
            dispatch_source_set_event_handler(_timer, ^{
                    struct tcp_pcb *pcb = self->_block->pcb;
                if (pcb == NULL) {
                        dispatch_source_cancel(self->_timer);
                        [self->_tunnel removeConnectionForKey:self->_identifie];
                } else {
                        tcp_tmr(self->_block);
                }
            });
                ...
                pbuf:(struct pbuf *)pbuf
{
    dispatch_async(_timerQueue, ^{
            self->_block->ip_data = ipdata;
            self->_block->tcpInfo = info;
            tcp_input(pbuf, self->_block);
    });
}
...
- (BOOL)syncSetDelegate:(id<ZPTCPConnectionDelegate>)delegate delegateQueue:(dispatch_queue_t)queue
{
    ...
    dispatch_sync(_timerQueue, ^{
            ...
            if (queue) {
                    ...
                    self->_delegateQueue = dispatch_queue_create("ZPTCPConnection.delegateQueue", NULL);
            }
            pcb_is_valid = TRUE;
        } else {
            pcb_is_valid = FALSE;
...
- (void)write:(NSData *)data
{
    dispatch_async(_timerQueue, ^{
            struct tcp_pcb *pcb = self->_block->pcb;
            if (pcb == NULL || self->_block->close_after_writing) {
            return;
        }
        err_t err = tcp_write(pcb, data.bytes, data.length, TCP_WRITE_FLAG_COPY);
        if (err == ERR_OK) {
                tcp_output(pcb, self->_block);
        } else {
            NSString *errDomain;
            if (err == ERR_CONN) {
...
{
    dispatch_async(_timerQueue, ^{
            self->_block->close_after_writing = 1;
    });
}
